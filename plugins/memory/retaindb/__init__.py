...
class _WriteQueue:
    """Durable async write queue. Survives crashes and container replacement."""
    ...
    def __init__(self, client: _Client, db_path: Path):
        ...
        conn.execute(
            f"CREATE TABLE IF NOT EXISTS pending (id {identity}, user_id TEXT, "
    ...
    def _get_conn(self) -> sqlite3.Connection:
        ...
        if conn is None:
            ...
            settings = resolve_database_settings(self._db_path)
            if settings.backend == "postgres":
                from hermes_cli.postgres_util import connect
                conn = self._local.conn = connect(settings, "retaindb_queue")
            else:
    ...
    def enqueue(self, user_id: str, session_id: str, messages: list) -> None:
        ...
        with self._shutdown_lock:
            ...
            conn = self._get_conn()
            statement = (
                "INSERT INTO pending (user_id, session_id, messages_json, created_at) "
                "VALUES (?,?,?,?)"
            )
            params = (user_id, session_id, json.dumps(messages, ensure_ascii=False), now)
            if getattr(conn, "backend", "sqlite") == "postgres":
                row_id = conn.execute(statement + " RETURNING id", params).fetchone()[0]
                conn.commit()
            else:
                row_id = self._execute(statement, params).lastrowid
