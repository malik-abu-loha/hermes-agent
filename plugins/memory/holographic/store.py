...
CREATE TABLE IF NOT EXISTS facts (
    ...
    content TEXT NOT NULL UNIQUE, category TEXT DEFAULT 'general', tags TEXT DEFAULT '',
    trust_score DOUBLE PRECISION DEFAULT 0.5, retrieval_count INTEGER DEFAULT 0,
    helpful_count INTEGER DEFAULT 0, created_at TEXT DEFAULT (CURRENT_TIMESTAMP::text),
    updated_at TEXT DEFAULT (CURRENT_TIMESTAMP::text), hrr_vector BYTEA);
CREATE INDEX IF NOT EXISTS idx_facts_search ON facts USING GIN
    (to_tsvector('simple', content || ' ' || tags));
CREATE INDEX IF NOT EXISTS idx_facts_trust ON facts(trust_score DESC);
CREATE INDEX IF NOT EXISTS idx_facts_category ON facts(category);
CREATE TABLE IF NOT EXISTS entities (
    entity_id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    name TEXT NOT NULL, entity_type TEXT DEFAULT 'unknown', aliases TEXT DEFAULT '',
    created_at TEXT DEFAULT (CURRENT_TIMESTAMP::text));
CREATE INDEX IF NOT EXISTS idx_entities_name ON entities(name);
CREATE TABLE IF NOT EXISTS fact_entities (
    fact_id BIGINT REFERENCES facts(fact_id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS memory_banks (
    ...
    bank_name TEXT NOT NULL UNIQUE, vector BYTEA NOT NULL, dim INTEGER NOT NULL,
    fact_count INTEGER DEFAULT 0, updated_at TEXT DEFAULT (CURRENT_TIMESTAMP::text));
"""
...
class MemoryStore:
    ...
    def __init__(self, db_path: "str | Path | None" = None, default_trust: float = 0.5, hrr_dim: int = 1024) -> None:
        ...
        settings = resolve_database_settings(self.db_path)
        ...
        with MemoryStore._shared_guard:
            ...
            if entry is None:
                ...
                if settings.backend == "postgres":
                    ...
                    conn = connect(settings, "holographic_memory")
                else:
    ...
    def add_fact(self, content: str, category: str = "general", tags: str = "") -> int:
        ...
        with self._lock:
            ...
            try:
                statement = "INSERT INTO facts (content, category, tags, trust_score) VALUES (?, ?, ?, ?)"
                params = (content, category, tags, self.default_trust)
                if getattr(self._conn, "backend", "sqlite") == "postgres":
                    fact_id = int(self._write(statement + " RETURNING fact_id", params).fetchone()[0])
                else:
                    fact_id = int(self._write(statement, params).lastrowid)
                ...
                if getattr(self._conn, "backend", "sqlite") != "postgres":
                    ...
                    import psycopg
                    if not isinstance(exc, psycopg.IntegrityError):
                        raise
    ...
    def _link_entities(self, fact_id: int, content: str) -> None:
        ...
        for name in self._extract_entities(content):
            self._write("INSERT INTO fact_entities (fact_id, entity_id) VALUES (?, ?) ON CONFLICT DO NOTHING",
    ...
    def _resolve_entity(self, name: str) -> int:
        ...
        statement = "INSERT INTO entities (name) VALUES (?)"
        if getattr(self._conn, "backend", "sqlite") == "postgres":
            return int(self._write(statement + " RETURNING entity_id", (name,)).fetchone()[0])
        return int(self._write(statement, (name,)).lastrowid)
