...
def sweep_failed_for_runtime(
    ...
    def claim(connection):
        ...
        for (
            ...
            if adapter_profile != expected_profile or not ledger._runtime_retryable(
                last_error
            ):
                continue
            if (
                ...
                continue
            if ledger.is_flood_error(
                last_error
            ) and current_time < ledger.flood_not_before(updated_at, last_error):
                continue
            cursor = connection.execute(
                """UPDATE delivery_obligations
...
def pending_flood_retries(now: Optional[float] = None) -> List[Dict[str, Any]]:
    ...
    for (
        ...
        if (
            not ledger.is_flood_error(last_error)
            or attempts >= ledger.MAX_ATTEMPTS
            or (current_time - created_at) > ledger.STALE_AFTER_SECONDS
        ):
            continue
        due = ledger.flood_not_before(updated_at, last_error)
