...
_INCREMENT_COUNTER_SQL = """
            ...
            DO UPDATE SET value = counter_aggregates.value + 1
            """
...
class SharedMetricsStore:
    ...
    def __init__(
        ...
        self._database_settings = resolve_database_settings(get_hermes_home() / "state.db")
        if self._database_settings.backend != "postgres":
    ...
    def _connection(
        ...
        if self._database_settings.backend == "postgres":
            ...
            connection = connect(self._database_settings, "shared_metrics")
            try:
                yield connection
            finally:
                connection.close()
    ...
    def _ensure_schema(self) -> None:
        ...
        with self._write(busy_timeout_ms=_SCHEMA_BUSY_TIMEOUT_MS) as connection:
            ...
            if getattr(connection, "backend", "sqlite") == "postgres":
                ...
                existing = table_columns(connection, "package_outbox")
            else:
                existing = {str(row["name"]) for row in connection.execute("PRAGMA table_info(package_outbox)")}
            ...
            for statement in _CREATE_CONSENT_TABLES_SQL:
                if getattr(connection, "backend", "sqlite") == "postgres" and "send_consent_windows" in statement:
                    statement = """CREATE TABLE IF NOT EXISTS send_consent_windows (
                        rowid BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                        opened_at TEXT NOT NULL, last_confirmed_at TEXT NOT NULL, closed_at TEXT)"""
                connection.execute(statement)
            connection.execute(
    ...
    def _install_id(self, connection: sqlite3.Connection) -> str:
        ...
        if row is None:
            connection.execute(
                "INSERT INTO telemetry_state(key, value) VALUES ('install_id', ?) ON CONFLICT(key) DO NOTHING",
                (str(uuid.uuid4()),),
            )
            row = connection.execute(query).fetchone()
        if row is None:
